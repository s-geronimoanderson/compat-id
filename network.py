...
class Network:
    ...
    def __init__(self, size=0):
        ...
        self.network = Matrix(size=(size, size))
        self.size = size
    ...
    def to_matrix(self):
        return Matrix(matrix=self.network)

    def to_1d_array_alternate(self):
        """Return an array with all rows concatenated."""
        return self.network.to_1d_array()

    def to_ijv(self):
        """Return three arrays in ijv (triplet) format."""
        return self.network.to_ijv()

    def to_coo_matrix(self):
        """Return a matrix in COOrdinate format."""
        return self.network.to_coo_matrix()

    def send(self, source, destination, payload):
        """Simulate sending the payload from the source to the destination."""
        subscript = (source, destination)
        self.network.set(subscript, (self.network.get(subscript) + payload))
    ...
    def many_to_many(self, destinations=None, payload=0, sources=None):
        ...
        for source in sources:
            if self.valid_process(source):
                for destination in destinations:
                    if self.valid_process(destination):
